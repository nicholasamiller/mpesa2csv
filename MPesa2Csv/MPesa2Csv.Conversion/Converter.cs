...
namespace MPesa2Csv.Conversion
{
    public static class Converter
    {

        public static  async Task<Stream> ConvertToCsv(string password, Stream mpesaPdf)
        {
            var readerProperties = new ReaderProperties().SetPassword(Encoding.UTF8.GetBytes(password));
            var pdfReader = new  PdfReader(mpesaPdf, readerProperties);
